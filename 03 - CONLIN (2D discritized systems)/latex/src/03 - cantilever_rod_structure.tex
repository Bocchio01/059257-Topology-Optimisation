\section{Topology optimization of a cantilever rods structure}
\label{sec:cantilever_rod_structure}
...
The goal of this section is to perform a topology optimization of the truss structure shown in Figure \ref{fig:truss_structure}, minimizing the compliance $C$ by acting on the cross-sectional areas of the rods.
...
As can be observed, the structure is composed of $N_{nodes} = 77$ nodes and $N_{rods} = 200$ rods.
...
For the case at hand, we suppose to have a vertical load $F = -6000N$ applied to node $49$ (lower right corner of the structure), and the rod's Young's modulus to be $E = 210 GPa$ (steel).
...
The parameter $\alpha$ is the volume fraction of the structure, defined as $\alpha = \frac{V_0}{V_{max}}$, where $V_{max} = \text{UB} \sum_{j=1}^{N_{rods}} l_j$ is the maximum volume of the structure.
...
Based on these definitions, we can redefine both the optimization problem $(P)_{nf}$ and the Lagrangian function $\mathcal{L}^{k}(\bm{x}, \lambda)$ as follows:
...
\begin{equation}
    \mathcal{L}^{k}(\bm{x}, \lambda) = r_0^k + \sum_{j \in \Omega_-} \left(\frac{q_{0j}^k}{x_j} + \lambda l_j x_j \right) - \lambda V_{max}
\end{equation}

Also, the solution of the dual optimization problem $(D)$ can be found by solving the following equation:

\begin{equation}
    \frac{\partial \mathcal{L}^k}{\partial x_j} = - \frac{q_{0j}^k}{x_j^2} + \lambda l_j = 0 \rightarrow x_j^* = \sqrt{\frac{q_{0j}^k}{\lambda l_j}}
\end{equation}
...
\begin{equation}
    \frac{\partial \phi^k(\lambda)}{\partial \lambda} = \sum_{j \in \Omega_-} l_j x_j^{(k+1)}(\lambda^*) - V_{max} = 0
    \label{eq:lambda_update}
\end{equation}

So basically, one can solve for the Lagrangian multiplier $\lambda$ by finding numerically the root of Equation \ref{eq:lambda_update}.
Once the value of $\lambda^*$ is found, we can update the design variables $\bm{x}(\lambda)$ using the projection operator defined in Equation \ref{eq:projection_operator}.
...
Red color indicates rods with a positive stress ($\sigma > 0$), while blue color indicates rods with a negative stress ($\sigma < 0$).
...
In particular, larger values of $\alpha$ lead to a denser structure, with more rods contributing to the load-bearing capacity of the structure.
On the other hand, smaller values of $\alpha$ lead to a less stiff structure, with increased compliance values.
...
This phenomenon, which is known as the \text{fully stressed condition}, occurs because the Lagrangian multiplier is directly proportional to the square stress in cases where the design variables stays within the bounds.
That is:

\begin{equation}
    \lambda = \frac{\sigma_j^2}{E} \rightarrow \sigma_j = \pm \sqrt{E \lambda}
    \label{eq:fully_stressed_condition}
\end{equation}

Equation \ref{eq:fully_stressed_condition} can be proved by considering the KKT conditions for the compliance minimization problem of Equation \ref{eq:compliance_minimization_problem} and imposing its Lagrangian function to be stationary with respect to the design variables $\bm{x}$:

\begin{equation}
    \frac{\partial \mathcal{L}(\bm{x}, \lambda)}{\partial x_j} = \frac{\partial C(\bm{x})}{\partial x_j} - \lambda l_j = 0 \rightarrow -\frac{\sigma_j^2}{E} l_j = \lambda l_j
\end{equation}

From the equation above, it's easy to recognize the fully stressed condition of Equation \ref{eq:fully_stressed_condition} which holds when the design variables are within the design bounds $x_j^{min} < x_j < x_j^{max}$.
