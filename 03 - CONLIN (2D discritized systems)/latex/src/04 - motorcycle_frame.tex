...
We are asked to perform the optimization of the structure when $\alpha = 0.1$, $LB = 0.001 [mm^2]$ and $UB = 700 [mm^2]$.
Moreover, given that we have to consider two load cases simultaneously, we are also given a weight factor $w_1$ and $w_2$ for each load case and we are asked to run the optimization for various values of $w_1$ and $w_2$.
