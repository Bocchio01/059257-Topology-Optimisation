...
In both the equations above, the design variables $\bm{x}$ can represent the material properties of the structure, such as the density of the material at each element of the structure, or the geometry of the structure, such as the cross-sectional area of the elements or the thickness of the elements.
...
\begin{equation}
    C_j = \frac{1}{2} \int \sigma_j \varepsilon_j dV_j = \frac{1}{2} \bm{u}_j^T \bm{k}_j \bm{u}_j
...
Based on Equation \ref{eq:conlin_compliance_minimization_problem}, we can also define the Lagrangian function $\mathcal{L}(\bm{x}, \lambda)$ as follows:

\begin{equation}
    \mathcal{L}^{k}(\bm{x}, \lambda) = f^{(C, k)}(\bm{x}) + \lambda \left( \sum_{j=1}^{n} l_j x_j - V_{max} \right)
    \label{eq:lagrangian_compliance_minimization}
\end{equation}

By doing so, we can define the dual optimization problem $(D)$ as follows:
...
Where, an explicit solution to the dual optimization problem $(D)$ can be found by solving the following equation:
