function x = TO4_distributed(nx, ny, alpha, p, rmin)
...
for ely = 1:ny
    for elx = 1:nx
        if sqrt((ely-ny/2.)^2+(elx-nx/3.)^2) < ny/3.
            passive(ely,elx) = 1;
            x(ely,elx) = 0.001;
        end
    end
...
while change > 0.01
    ...
    x = optimality_criteria(nx, ny, x, alpha, dc, passive);
...
function xnew = optimality_criteria(nx, ny, x, alpha, dc, passive)
